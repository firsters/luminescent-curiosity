...
export default function ItemCard({ item, fridgeName, onClick, onConsume }) {
  const { days, badge, itemBgClass } = useMemo(() => {
    ...
    if (days < 0) {
      badge = { text: `D+${Math.abs(days)}`, color: 'bg-gray-100 text-gray-500' };
    } else if (days <= 3) {
      badge = { text: `D-${days}`, color: 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400' };
    } else if (days <= 7) {
      badge = { text: `D-${days}`, color: 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400' };
    } else {
      badge = { text: `여유`, color: 'text-[#0e1b12] dark:text-white font-bold' };
    }
    ...
    >
      ...
      <div className="flex flex-1 flex-col justify-center min-w-0">
        ...
        <div className="flex items-center gap-1.5 mb-1">
          <p className="text-[#0e1b12] dark:text-white text-base font-bold leading-tight truncate">{item.name}</p>
