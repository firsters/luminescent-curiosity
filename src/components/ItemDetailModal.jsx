...
export default function ItemDetailModal({ item, fridgeName, onClose, onDelete, onConsume, onEdit }) {
  const navigate = useNavigate();

  if (!item) return null;

  // Helper to calculate days until expiry (matching InventoryList logic)
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
        <div className="w-full max-w-md bg-surface-light dark:bg-surface-dark rounded-3xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="relative h-64 w-full bg-gray-100 dark:bg-black/20">
                {item.photoUrl ? (
                    <img src={item.photoUrl} alt={item.name || 'Food Item'} className="w-full h-full object-cover" />
                ) : (
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5">
                        ...
                        <p className="font-bold text-[#0e1b12] dark:text-white">
                            {item.expiryDate ? new Date(item.expiryDate).toISOString().split('T')[0] : '미지정'}
                            {item.expiryDate && (
                    ...
                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5">
                        ...
                        <p className="font-bold text-[#0e1b12] dark:text-white">
                            {item.addedDate ? new Date(item.addedDate).toISOString().split('T')[0] : '-'}
                        </p>
                    </div>
